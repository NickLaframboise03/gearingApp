...
def interp_lin(x: float, xp: Sequence[float], fp: Sequence[float]) -> float:
    ...
    """Simple 1D linear interpolation that works with lists or numpy arrays.
    ...
    float
        ...
        Interpolated value at x, clamped to [xp[0], xp[-1]].
    ...
    # Support numpy arrays and Python sequences
    ...
    try:
        ...
        # If xp/fp are scalars, just return fp
    ...
    if n_xp == 0:
        ...
        return float(fp[0]) if n_fp > 0 else 0.0


    if n_fp != n_xp:

        raise ValueError("interp_lin: xp and fp must have the same length")
