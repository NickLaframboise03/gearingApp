...
def apply_dark(fig: go.Figure, height=None, title=None):
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            title_font=dict(color=DARK_AXIS),
                    ...
                    dbc.Col(
                        html.Div(
                            [
                                ...
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Mass [kg]")),
                                        dbc.Col(num("mass", v["m"])),
                                    ]
                                ),
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Cd")),
                                        dbc.Col(num("cd", v["Cd"], step=0.01)),
                                    ]
                                ),
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Af [m²]")),
                                        dbc.Col(num("af", v["Af"], step=0.01)),
                                    ]
                                ),
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Air ρ [kg/m³]")),
                                        dbc.Col(
                                            num(
                                                "rho_air",
                                                v["rho_air"],
                                ...
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Fuel ρ [kg/L]")),
                                        dbc.Col(
                                            num(
                                                "rho_fuel",
                                                v["rho_fuel"],
                                ...
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Fuel LHV [MJ/kg]")),
                                        dbc.Col(
                                            num(
                                                "lhv_mjpkg",
                                                v["LHV"] / 1e6,
                                    ...
                                    [
                                        dbc.Col(
                                            dcc.Checklist(
                                                ...
                                                options=[
                                                    {
                                                        "label": " Automatic (torque converter)",
                                                        "value": 1,
                                                    }
                                                ],
                                                value=[
                                ...
                                dbc.Row(
                                    [
                                        dbc.Col(
                                            html.Label("Final drive")
                                        ),
                                        dbc.Col(
                                            num("fd", v["fd"], step=0.01)
                                        ),
                                    ]
                                ),
                                dbc.Row(
                                    [
                                        dbc.Col(
                                            html.Label("Tire width [mm]")
                                        ),
                                        dbc.Col(num("tire_w", v["tire_w"])),
                                    ]
                                ),
                                dbc.Row(
                                    [
                                        dbc.Col(html.Label("Aspect [%]")),
                                        dbc.Col(
                                            num(
                                                "tire_ar",
                                                v["tire_ar"],
        ...
        dcc.Tabs(
            ...
            children=[
                dcc.Tab(
                    ...
                    value="tab1",
                    children=html.Div(
                        id="page-tab1", children=tab1_layout(v0, eng0)
                    ),
                ),
                dcc.Tab(
                    ...
                    value="tab2",
                    children=html.Div(
                        id="page-tab2", children=tab2_layout(v0)
                    ),
                ),
                dcc.Tab(
                    ...
                    value="tab3",
                    children=html.Div(
                        id="page-tab3", children=tab3_layout(v0, eng0)
                    ),
                ),
                dcc.Tab(
                    ...
                    value="tab4",
                    children=html.Div(
                        id="page-tab4", children=tab4_layout(v0)
                    ),
                ),
            ],
        ),
...
@callback(
    ...
    State("af", "value"),
    State("rho_air", "value"),
    State("rho_fuel", "value"),
    State("lhv_mjpkg", "value"),
    State("g", "value"),
    State("eta_dl", "value"),
    State("tire_w", "value"),
    State("tire_ar", "value"),
    State("tire_rim_in", "value"),
    State("fd", "value"),
    State("gear_ratios", "value"),
    State("idle_rpm", "value"),
    State("redline_rpm", "value"),
    State("auto-trans-enabled", "value"),
    State("stall-speed-rpm", "value"),
...
def apply_or_reset(
    ...
    if ctx_id == "btn-reset":
        ...
        v = {
            ...
            "LHV": float(lhv_mjpkg) * 1e6,
            "g": float(g),
            "eta_dl": float(eta_dl),
            "tire_w": float(tire_w),
            "tire_ar": float(tire_ar),
            "tire_rim_in": float(tire_rim_in),
            "fd": float(fd),
            "gears": gears,
    ...
    label = (
        f"re = {v['re']:.3f} m | top-gear @ redline ≈ "
        f"{v['top_speed_kmh']:.1f} km/h"
    )
    opts = [
        {"label": f"G{i+1}", "value": i + 1}
        for i in range(len(v["gears"]))
    ]
    return (
        v,
        label,
        opts,
        [len(v["gears"])] if v["gears"] else [],
    )
...
def manage_vehicle_profiles(
    ...
    if triggered == "btn-save-profile":
        ...
        if engine_value is not None:
            engine_file = str(engine_value)
            if eng_state:
                for item in eng_state.get("items", []):
                    if item.get("name") == engine_value:
                        engine_file = (
                            item.get("source_file") or engine_file
                        )
                        break
        ...
        new_profiles = [
            entry
            for entry in profiles
            if entry.get("name") != selected_value
        ]
    ...
    options = [
        {"label": entry.get("name"), "value": entry.get("name")}
        for entry in profiles
        if entry.get("name")
    ]
                ...
                if target_idx is None and desired:
                    ...
                    if candidate.exists():
                        try:
                            ...
                            print(
                                f"Failed to load engine '{desired}' for profile '{profile_name}':",
                                ex,
                            )
...
def update_gearing(active_tab, v, M, _nclicks, loads_txt):
    ...
    gear_colors = [
        f"hsl({int(360 * i / nG)},60%,60%)" for i in range(nG)
    ]
    ...
    fig_speedrpm.add_hline(
        y=v["redline_rpm"],
        line=dict(color="#ff5d5d", dash="dash"),
        annotation_text="Redline",
    )
    apply_dark(
        fig_speedrpm,
    ...
    apply_dark(
        fig_torque,
        title="Engine torque vs vehicle speed (per gear) at normalized loads",
    )
    apply_dark(
        fig_accel,
    ...
    apply_dark(
        fig_force,
        title="Available tractive force (λ=1) and resistance vs speed",
    )
...
def update_map_and_time(
    ...
    if markers_txt:
        try:
            raw = [
                float(x)
                for x in str(markers_txt).replace(",", " ").split()
            ]
            markers = sorted(
                {
                    m
    ...
    Zeta = (
        np.array(M.get("eta_map"), dtype=float)
        if ("eta_map" in M)
        else None
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=_arr(M["wot_speed_radps"]) * 60 / (2 * np.pi),
    ...
    fig.add_trace(
        go.Scatter(
            x=_arr(M["ct_speed_radps"]) * 60 / (2 * np.pi),
    ...
    if show_cruise and 1 in show_cruise and cruise_gear:
        rpm, Treq, v_kmh = C.steady_cruise_curve(
            v, M, int(cruise_gear) - 1
        )
        ...
        if ok.any():
            ...
            if vk.size:
                marks = np.arange(
                    step * np.ceil(vk.min() / step),
                    step * np.floor(vk.max() / step) + 1e-9,
                    step,
                )
                annotations = []
                for s in marks:
    ...
    for i, S in enumerate(seqs or []):
        ...
        if ts.size and vk.size and ts.size == vk.size:
            ...
            # Add markers at requested distances
            for m in markers:
...
def fe_plot(active_tab, lbl_step, show_gears, v, M):
    ...
    for gi in range(nG):
        ...
        v_idle = max(
            0,
            (v["idle_rpm"] * Krpm2kmh) / v["gears"][gi],
        )
        ...
        if mask.any():
            ...
            lbs = np.arange(
                lbl_step * np.ceil(v_idle / lbl_step),
                lbl_step * np.floor(v_redl / lbl_step) + 1e-9,
                lbl_step,
            )
            for s in lbs:
    ...
    apply_dark(
        fig, height=720, title="Steady-state fuel consumption per gear"
    )
    return fig
